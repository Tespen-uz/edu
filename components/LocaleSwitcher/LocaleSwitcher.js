...
export default function LocaleSwitcher() {
  ...
  const [selected, setSelected] = useState(langs[langs.indexOf(locale)]);

  const options = locales?.find((cur) => cur !== locale);

  React.useEffect(() => {
    const shouldChange = isReady && locale !== selected;

    if (shouldChange) {
      push(
        {
          pathname: pathname,
          query: { ...query, locale: selected },
        },
