...
function ExamCardIteam({ item }) {
  return (
    <Link href={`/register/${item.id}`}>
      <a className="block border-2 border-emerald-200 rounded-md w-full sm:w-10/12 md:w-[48%] lg:w-[32%] p-2 my-4 ">
        <div>
        ...
        <div className="flex justify-end">
          ...
          <button className="w-full text-lg text-center font-bold p-2  text-white block mt-4 bg-emerald-300 rounded-md transition-all hover:scale-[0.97]">
            Start Now
          </button>
      </a>
    </Link>
